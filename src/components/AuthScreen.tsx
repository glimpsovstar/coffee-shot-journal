...
interface AuthScreenProps {
  ...
  onSignInWithOAuth: (provider: OAuthProviderId) => Promise<void>;
}
...
export function AuthScreen({
  ...
  const [submittingOAuth, setSubmittingOAuth] = useState<OAuthProviderId | null>(null);
  const [localError, setLocalError] = useState<string | null>(null);

  const message = localError ?? error;
  const busy = submittingPasskey || submittingOAuth !== null;
      ...
      <div className="landing__layout">
        <section className="landing__hero" aria-labelledby="landing-tagline">
          ...
          <ul className="landing__features">
            <li>Log shots with grind, dose, yield, and tasting notes</li>
            <li>Catalogue beans with label photos and roast details</li>
            <li>Sync across phone, laptop, and desktop with your account</li>
          </ul>
        </section>

        <section className="auth-panel landing__auth" aria-labelledby="auth-heading">
          <h2 id="auth-heading">Create account or sign in</h2>
          ...
          <div className="oauth-buttons">
            {OAUTH_PROVIDERS.map((provider) => (
              ...
              >
                ...
                <span>
                  {submittingOAuth === provider.id ? 'Redirecting…' : provider.label}
                </span>
              </button>
            ))}
          </div>
...
function OAuthIcon({ provider }: { provider: OAuthProviderId }) {
  switch (provider) {
    case 'google':
      return (
        <svg className="oauth-button__icon" viewBox="0 0 24 24" aria-hidden="true">
          <path
            fill="#4285F4"
